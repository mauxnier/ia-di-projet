...
# Train Random Forest Model and evaluate on all combinations
...
try:
    for i, train_combination in enumerate(train_combinations):
        ...
        # Train Random Forest Model for the subset
        rf_model.fit(X_train_subset_scaled, y_train_subset_binary)
        y_pred_subset = rf_model.predict(X_test_scaled)
        ...
        print(f"\nRandom Forest Classification Report on Training Combination {i + 1}:")
...
# Train Random Forest Model on the full training set
rf_model.fit(X_train_scaled_full, y_train_binary_full)

# Print overall accuracy, precision, recall, and F1-score on the full training set
y_pred_full = rf_model.predict(X_test_scaled)
